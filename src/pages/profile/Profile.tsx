...
export default function Profile() {
  ...
  // ✅ IMPORTANT: update to your real dashboard routes
  const workspacePath = useMemo(() => {
    if (user?.role === "hr") return "/hr/dashboard";        // change if needed
    if (user?.role === "manager") return "/manager/profile"; // change if needed
    return "/employee/profile";                             // change if needed
  }, [user?.role]);
    ...
    <div style={{ padding: 18, maxWidth: 1080, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
        <div>
          ...
          <div style={{ color: "#64748b", marginTop: 4 }}>View your account, identity, and platform status.</div>
      ...
      {status === "error" && (
        <Card title="Error" subtitle="We couldn't load your profile">
          ...
          <div style={{ color: "#64748b", marginTop: 8 }}>If this keeps happening, your token might be expired.</div>
        </Card>
      )}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.35fr 0.65fr", gap: 14 }}>
          ...
          <div style={{ display: "grid", gap: 14 }}>
            <Card title="Quick Actions" subtitle="Common actions">
              <Button as="link" to={workspacePath} variant="primary">
                Go to workspace
              </Button>

              <Button variant="outline" onClick={() => copy(user._id)}>
                Copy User ID
              </Button>
